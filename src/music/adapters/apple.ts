...
class AppleAdapter implements MusicProvider {
  ...
  async resolveTrack(track: CanonicalTrack): Promise<ProviderTrackHandle> {
    ...
    // Apple song IDs are storefront-specific: the same recording has a
    // different ID in the US, Canada, UK catalogs. Looking up by ISRC
    // (which IS global) against the user's actual storefront guarantees
    // we get an ID that will resolve for their subscription.
    const sf = this.music.storefrontId || 'us';
    const isrc = track.id;
    const resp = (await this.music.api.music(
      `v1/catalog/${sf}/songs`,
      { 'filter[isrc]': isrc, limit: 1 },
    )) as AppleApiResponse;
    const item = resp?.data?.data?.[0];
    if (!item) {
      throw new Error(
        `Apple Music: no track for ISRC ${isrc} in storefront ${sf}`,
      );
    }
    return {
